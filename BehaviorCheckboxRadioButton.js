$(window).on('load', function () {
  ...
  $('.parametrs-content-items').each(function () {
    ...
    var checkboxHiddenOption = currentGroup.find('.dublicate-checkbox-field .options-name-item-dublicate').text().trim();
    var checkboxHiddenPrice = currentGroup.find('.dublicate-checkbox-field .options-name-price-value-dublicate').text().trim();
    ...
    currentGroup.find('.checkbox-button-field-2 .options-name-item').each(function () {
      ...
      // Сравнить текстовые значения с учетом регистра
      if (checkboxCurrentOption.toLowerCase() === checkboxHiddenOption.toLowerCase()) {
        ...
        // Также установить значение цены для чекбокса
        var checkboxPrice = parseFloat(checkboxHiddenPrice) || 0;
        // Добавьте код для использования значения цены по своему усмотрению
